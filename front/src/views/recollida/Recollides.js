...
const Users = () => {
  ...
  const pageChange = newPage => {
    currentPage !== newPage && history.push(`/Recollides?page=${newPage}`)
  }
  ...
  return (
    ...
    <Row noGutters className="page-header py-4">
      <PageTitle sm="4" title="Recollides" subtitle="Recollida" className="text-sm-left" />
    </Row>
        ...
        <CCard>
          ...
          <CDataTable
            ...
            fields={[
              ...
              'date', 'time', 'idVehicle', 'realitzada'
            ]}
            hover
            striped
            itemsPerPage={10}
            activePage={page}
            clickableRows
            onRowClick={(item) => history.push(`/Recollida/${item.id}`)}
            scopedSlots = {{
              'realitzada':
                (item)=>(
                  <td>
