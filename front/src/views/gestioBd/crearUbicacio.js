...
function handleClickButton(p1){
	 ...
	 const requestOptions = {
        ...
        body: JSON.stringify({ tag: p1.tag})
    };
    fetch('/addUbicacio', requestOptions)
    .then(response => {
      if(response.ok){
				setState({tag:''})
    ...
    <React.Fragment>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={4}>
          <TextField onChange={handleChange} name="tag" required id="cardName" label="Tag" fullWidth autoComplete="cc-name" />
        </Grid>
