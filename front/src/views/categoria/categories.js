...
const Users = () => {
  ...
  return (
    ...
    <Row noGutters className="page-header py-4">
      <PageTitle sm="4" title="Categories" subtitle="Categoria" className="text-sm-left" />
    </Row>
      ...
      <CCol xl={12}>
        <CCard>
          ...
          <CDataTable
            items={items}
            fields={[
              { key: 'nom', _classes: 'font-weight-bold' }
            ]}
            hover
            striped
            itemsPerPage={10}
            activePage={page}
            clickableRows
            onRowClick={(item) => history.push(`/categoria/${item.nom}`)}
            /*scopedSlots = {{
              'realitzada':
                (item)=>(
                  <td>
