...
const router = app => {
  ...
  app.post('/login', (request, response) => {
    ...
    const conn = getConnection();
    conn.query("SELECT id FROM Usuari WHERE mail=? AND AES_DECRYPT(UNHEX(contrasenya),'willyrex')=?",[mail,password],(err,res) => {
      if (err) {
        console.error(err);
        conn.end();
...
// POST REQUESTS
	...
	app.post('/sendCode', (request, response) => {
			...
			url='scp /etc/agent/codes/{} marcmila@{}:/etc/agent/codes/'.format(file,ip)
...
//modi id
	...
	app.delete('/deleteUbicacio', (request,response) => {
		...
		conn.query("delete from Ubicacio where TAG=?", [id], (err,res) => {
			...
			response.status(200).send({message: 'Ubi eliminat correctament'});
			conn.end();
		});
	});
